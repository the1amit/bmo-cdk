...
export class AppTier extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AppTierProps) {
    ...
    // Define Docker image asset if  Docker image is not publicly available
    ...
    const container = taskDefinition.addContainer('WooCommerceContainer', {
      image: ecs.ContainerImage.fromRegistry('woocommerce/woocommerce'),
     // image: ecs.ContainerImage.fromDockerImageAsset(imageAsset), // If using local Docker Image path
      memoryLimitMiB: 512,
      cpu: 256,
      environment: {
